...
struct CameraView: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading) {
                // Connect / disconnect + status + share
                HStack {
                ...
                // Black and white level steppers
                VStack {
                    Stepper {
            ...
            // Right column: pixel grid + capture progress
            VStack(spacing: 4) {
                self.imageGrid
    ...
    private var imageGrid: some View {
        GeometryReader { geometry in
            let pixelSize = geometry.size.width / CGFloat(CameraViewViewModel.width)
            let columns = Array(repeating: GridItem(.fixed(pixelSize), spacing: 0), count: CameraViewViewModel.width)
        ...
        .aspectRatio(CGFloat(CameraViewViewModel.width) / CGFloat(CameraViewViewModel.height), contentMode: .fit)
    }
}
